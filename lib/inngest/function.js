  ...
  async ({ step }) => {
    ...
    for (const budget of budgets) {
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        // Calculate total expenses for ALL accounts
...
export const sendTestEmail = inngest.createFunction(
  { id: "send-test-email", name: "Send Test Email" },
  ...
  async ({ event, step }) => {
    await step.run("send-test-email", async () => {
      await sendEmail({
        ...
        react: EmailTemplate({
          ...
          type: "test",
          data: {
            message: "This is a test email to verify that Inngest and Resend are working correctly!",
    ...
    return { success: true, email: event.data.email };
  }
);
